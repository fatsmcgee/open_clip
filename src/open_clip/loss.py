...
class MatryoshkaOperator(nn.Module):
    # NOTE(ebenj): This is basically taken from JinaAI with small modifications
    def __init__(
        self,
        loss: nn.Module,
        full_embed_dim: int,
        ...
        self._full_embed_dim = full_embed_dim

        assert all(dim <= full_embed_dim for dim in dims), (
            'Some Matryoshka subspaces have a higher dimensionality than '
            'the full embedding space!'
        )
        if full_embed_dim not in self._dims:
            logging.warning(
                f'The full embedding dimensionality {full_embed_dim} is not included in the '
                'Matryoshka subspaces!'
            )

    def _is_feature_tensor(self, obj):
        # Essentially, if the tensor is BxD where D is embedding dim,
        # assume it is a model output embedding (text or visual)
        return (
            torch.is_tensor(obj)
            and torch.is_floating_point(obj)
            and len(obj.shape) == 2
            and obj.shape[1] == self._full_embed_dim
        )
